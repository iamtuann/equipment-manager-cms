<template>
  <v-container>
    ...
    <v-row class="mt-5">
      ...
      <v-col cols="12" sm="6">
        <p>Ngày đề xuất: {{ formatDate(new Date(formData.proposalDate)) }}</p>
      </v-col>
      <v-col cols="12" sm="6">
...
async function handleReject() {
  try {
    const response = await proposalReceiptStore.changeStatus(route.params.id, 2);
    toast.success("Đã từ chối phiếu đề xuất");
...
async function hanldeApprove() {
  try {
    const response = await proposalReceiptStore.changeStatus(route.params.id, 1);
    toast.success("Đã chấp nhận phiếu đề xuất");
    router.replace({name: "ProposalReceipts"});
  } catch (error) {
    console.log(error);
    toast.error(error?.response.data.message);

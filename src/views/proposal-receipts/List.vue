    ...
    <v-expansion-panels v-model="expansionPanel">
      <v-expansion-panel>
        <v-expansion-panel-title static>
          <span class="font-weight-medium text-h6">Phiếu đề xuất thiết bị</span>
              ...
              <v-col cols="12" sm="6" md="4" lg="3">
                <v-menu v-model="openProposalDate" offset-y :close-on-content-click="false">
                  <template v-slot:activator="{ props }">
                    <v-text-field v-bind="props" density="compact" :modelValue="proposalDateFormat"
                      append-inner-icon="mdi-calendar-month" label="Ngày đề xuất" clearable readonly
                      @click:clear="formSearch.proposalDate = null"></v-text-field>
                  </template>
                  <v-date-picker v-model="formSearch.proposalDate" no-title show-adjacent-months color="primary"
                    hide-weekdays hide-header @update:modelValue="openProposalDate = false">
                  </v-date-picker>
                </v-menu>
              </v-col>
              <v-col cols="12" sm="6" md="4" lg="3">
    ...
    <v-card class="mt-4">
      <v-card-item>
        <div class="d-flex justify-space-between align-center">
          ...
          <v-btn append-icon="mdi-plus" :to="{name: 'CreateProposalReceipt'}"
            v-if="authStore.isLDK()"
          >
            Tạo phiếu
          </v-btn>
        </div>
      ...
      <v-card-item>
        ...
        >
          ...
          <template v-slot:item.proposalDate="{ item }">
            {{ formatDate(new Date(item.proposalDate)) }}
          </template>
          ...
          <template v-slot:item.actions="{ item }">
            <div class="d-flex justify-center ga-2">
              <!-- <v-btn v-if="item.status == 0" size="x-small" icon color="info" :to="{ name: 'UpdateProposalReceipt', params: { id: item.id } }">
                <v-icon>mdi-pencil-outline</v-icon>
              </v-btn> -->
              <v-btn v-if="canDelete(item)" size="x-small" icon color="error" @click="handleDelete(item)">
                <v-icon>mdi-trash-can-outline</v-icon>
              </v-btn>
              <v-btn size="x-small" icon :to="{name: 'DetailProposalReceipt', params: {id: item.id}}">
        ...
        <template>
          <v-row justify="center">
            <v-dialog v-model="deleteDialog" persistent width="800">
              <v-card>
                <v-toolbar color="error" title="Xóa phiếu đề xuất"></v-toolbar>
                <v-card-text>
...
import { useProposalReceiptStore, useAuthStore } from "@/stores";
...
const proposalDateFormat = computed(() => formatDate(formSearch.proposalDate));
...
const dataTable = reactive({
  ...
  headers: [
    ...
    { title: "Ngày đề xuất", key: "proposalDate", align: "center", sortable: true },
...
async function getData({ page, itemsPerPage, sortBy }) {
  ...
  try {
    const { code, status, proposalDate, departmentId, createdBy, createdAt, approvedAt } = formSearch;
    const key = sortBy[0]?.key || null;
    const orderBy = sortBy[0]?.order || null;
    const response = await proposalReceiptStore.search(
      code, status, proposalDate, departmentId, createdBy, createdAt, approvedAt, page, itemsPerPage, key, orderBy
    );
...
async function deleteData(id) {
  try {
    const response = await proposalReceiptStore.delete(id);
